...
test('WPKL Awam - Permohonan SiFUS,PSIFUS,PPBB', async ({ page }) => {
  ...
  await page.waitForTimeout(3000);
  await page.getByRole('button', { name: 'Choose File' }).setInputFiles('C:/playwright/Document/JadualWP.xlsx');
  await page.waitForTimeout(5000);
  await page.getByRole('button', { name: 'Seterusnya' }).click();
  await page.waitForTimeout(5000);
  await page.getByRole('button', { name: 'Seterusnya' }).click();
  ...
  await page.locator('#username').fill('noraini@ptgwp.gov.my');
 ...
 // Back To Awam
  ...
  await page.locator('[id="hasilBayaranOnlineBaseForm:paymentOnline:bank-selected_24"]').click(); // Pilih SBI Bank A
  await page.waitForTimeout(3000);
  await page.getByRole('button', { name: 'Seterusnya' }).click();
  await page.waitForTimeout(3000);
