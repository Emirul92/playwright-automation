...
test('WPKL Permohonan Urusan PT, PLMS, PRU, dan PSBS dari Portal Awam sehingga janaan ID Permohonan selepas membuat bayaran di SPOC', async ({ page }) => {
  ...
  const filePath = path.resolve("C:/Users/Amalina/playwright/Document/Testing.pdf");
  const imgPath = path.resolve("C:/Users/Amalina/playwright/Document/KegunaanTesting.tiff");
 ...
 await page.getByRole('button', { name: 'Bayar' }).click();
  ...
  await page.getByRole('button', { name: 'Seterusnya' }).click();
  await page.waitForTimeout(3000);
  await page.locator('[id$="bank-selected"]').click();//guna nama item better dari id yg dijana oleh primefaces - editor emirul@22/7/26
  await page.waitForTimeout(1000);
  //await page.locator([id="centerForm:mainTab:tabItem7:awam-bayaran-online:paymentOnline:bank-selected_24"]).click();
  await page.locator('li[data-label="SBI Bank A"]').click();//guna nama item better dari id yg dijana oleh primefaces - editor emirul@22/7/26
  await page.waitForTimeout(1000);
  await page.getByRole('button', { name: 'Seterusnya' }).click();
  await page.waitForTimeout(3000);
