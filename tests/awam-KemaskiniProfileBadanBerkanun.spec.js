...
test('WPKL Awam - Kemaskini Profile Badan Berkanun', async ({ page }) => {
  ...
  await page.locator('iframe[title="eTanahAwam"]').contentFrame().getByRole('textbox', { name: 'ID Pengguna' }).fill('nawawi@mara.gov.my');
  ...
  // Menu Selection
  await page.locator('.menu-10').dispatchEvent('click');
  await page.waitForTimeout(1000);

  // Kemaskini Profile
  await page.getByRole('textbox', { name: 'Katalaluan Lama :' }).fill('etanah123#');
  await page.waitForTimeout(1000);
  await page.getByRole('textbox', { name: 'Alamat Berdaftar :' }).fill('20260219001513');
  await page.waitForTimeout(1000);
